...
-(void)viewWillAppear:(BOOL)animated{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onReadData:) name:notify_key_CZJK_did_read_history_data object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onReadFinish:) name:notify_key_CZJK_read_history_data_finish object:nil];
...
-(void)refreshInfoLabel{
    ...
    self.infolabel.text = [NSString stringWithFormat:@"Last Read HistoryDataTime: %f\n%@",lastsporttime,
...
-(void)onReadData:(NSNotification*)notify{
    ...
    int hour = [[param objectForKey:CZJK_NOTIFY_PARAM_HOUR] intValue];
    int minute = [[param objectForKey:CZJK_NOTIFY_PARAM_MINUTE] intValue];
    
    int mode = [[param objectForKey:CZJK_NOTIFY_PARAM_HISTORY_DATATYPE] intValue];
    int step = [[param objectForKey:CZJK_NOTIFY_PARAM_STEP] intValue];
    int cal = [[param objectForKey:CZJK_NOTIFY_PARAM_CAL] intValue];
    int dist = [[param objectForKey:CZJK_NOTIFY_PARAM_DISTANCE] intValue];
    NSString* str = [NSString stringWithFormat:@"date->[%d-%d-%d %.2d:%.2d:00],mode->[%d],step->[%d],cal->[%d],duration->[%d]",year,month,day,hour,minute,mode,step,cal,dist];
